...
using TepayLink.Sdisco.Web.Areas.Admin.Models.CustomizableDashboard;
...
{
    public abstract class CustomizableDashboardControllerBase : SdiscoControllerBase
    {
        ...
        public async Task<PartialViewResult> AddWidgetModal(string dashboardName, string pageId)
        {
            var userDashboard = await DashboardCustomizationAppService.GetUserDashboard(
                new GetDashboardInput
                {
                    ...
                    Application = SdiscoDashboardCustomizationConsts.Applications.Mvc
                }
            );
